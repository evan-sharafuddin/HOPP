...
class LayoutOptAEP(LayoutOptInterface):
    def objective(self, verbose) -> (np.array, np.array): 
        x_in = self.x.reshape((-1, 2))
        ...
        AEP, dAEP = self._eval_model(x_graph_tuple)

        dAEP = dAEP.numpy()/np.array([[75000., 85000., 15.]]) # unnorm x.nodes
        dAEP = dAEP[:, :2] # remove third row, yaw isn't used in this optimization
        dAEP = np.sum(dAEP.reshape((self.num_simulations, self.num_turbines * 2)), axis=0)

        if verbose:
            print(f'ITERATION {self._opt_counter}')
            print('AEP: ', float(AEP))
            print('dAEP:\n', dAEP)
            print('x:\n', x_in)
            print()
    ...
    @tf.function
    def _eval_model(self, x) -> (np.array, np.array): 
        with tf.GradientTape() as tape:
            tape.watch(x.nodes)

            # TODO
            plant_power = 5e8 * self.model(x).globals[:, 0] / 1e6 # unnorm power
