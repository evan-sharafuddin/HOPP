...
class ConcretePlant:
    ...
    def run_profast_for_cement(self):
        ...
        * scale with plant capacity
        ...
        maintenance_equip = 0.025 * tpc
        operational_labor = 5.5 
        ### CEMCAP SPREADSHEET
        ...
        local_tax = 0.8
        # not used in pf
        total_fixed_opex = maintenance_equip + operational_labor + admin_support + insurance + local_tax + maintenance_labor
        
        print(f'variable opex: {total_var_opex}')
        print(f'fixed opex: {total_fixed_opex}')
        ...
        pf.set_params('debt interest rate',0.08) # 0.0489
        ...
        pf.add_capital_item(name='packaging plant, conveyor, loading, storing',cost=self.eur_to_usd(packaging_conveyor_loading_storing,1e6),\
                            depr_type="MACRS",depr_period=7,refurb=[0])
        pf.add_capital_item(name='mill, silo',cost=self.eur_to_usd(coal_mill_silo,1e6),\
                            depr_type="MACRS",depr_period=7,refurb=[0])
        ...
        pf.add_fixed_cost(name="Annual Operating Labor Cost",usage=1,unit='$/year',\
                          cost=self.eur_to_usd(operational_labor * cem_production * 1e6 * capacity_rate,1),escalation=gen_inflation)
        ...
        pf.add_fixed_cost(name="Maintenance Labor Cost",usage=1,unit='$/year',\
                          cost=self.eur_to_usd(maintenance_labor * cem_production * 1e6 * capacity_rate,1),escalation=gen_inflation)
        pf.add_fixed_cost(name="Administrative & Support Labor Cost",usage=1,unit='$/year',\
                          cost=self.eur_to_usd(admin_support * cem_production * 1e6 * capacity_rate,1),escalation=gen_inflation)
        ...
        pf.add_feedstock(name='Raw materials',usage=1.0,unit='unit per ton cem',cost=self.eur_to_usd(raw_materials * raw_meal_cli_factor,1),escalation=gen_inflation)
        pf.add_feedstock(name='fossil fuel',usage=1,unit='unit per ton cement',cost=self.eur_to_usd(5.2, 1),escalation=gen_inflation)
        pf.add_feedstock(name='alternative fuel',usage=1,unit='unit per ton cem',cost=self.eur_to_usd(alt_fuel,1),escalation=gen_inflation)
        pf.add_feedstock(name='power, kiln plant + grinding',usage=1,unit='unit per ton cem',cost=self.eur_to_usd(power_kiln_plant_grinding,1),escalation=gen_inflation)
        pf.add_feedstock(name='process water',usage=1,unit='unit per ton cem',cost=self.eur_to_usd(process_water,1),escalation=gen_inflation)
        pf.add_feedstock(name='Misc.',usage=1,unit='unit per ton cem',cost=self.eur_to_usd(misc,1),escalation=gen_inflation)
...
# NOTE: values are similar to spreadsheet ($61.13 is what the spreadsheet had). 
# reasons for discrepacy could be differences in capital cost, labor & maintenance, etc
#   need to work in values from the spreadsheet instead of from the paper a bit more
